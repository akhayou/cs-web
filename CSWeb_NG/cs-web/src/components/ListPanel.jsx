...
export default function ListPanel({
    ...
    isMobile,
    isRTL,
    onShowListChange, // <-- callback prop
}) {
    ...
    // Prevent duplicate calls by remembering the previous value
    const previousShowList = useRef(showList);

    // Trigger callback whenever showList changes
    useEffect(() => {
        // Only fire if the value actually changed
        if (previousShowList.current !== showList) {
            previousShowList.current = showList;
            onShowListChange?.(showList);
        }
    }, [showList, onShowListChange]);
